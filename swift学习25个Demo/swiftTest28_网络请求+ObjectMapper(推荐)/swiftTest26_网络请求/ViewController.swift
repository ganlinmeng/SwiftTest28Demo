...
//        test6()
    ...
    func test4()  {
        ...
        Alamofire.request("http://0.0.0.0:tuicool@api.tuicool.com/api/articles/hot.json", method: .get, parameters: ["cid":0,"size":30], encoding:URLEncoding.default, headers: nil).responseJSON { (response) in
    ...
    func test6()  {
        Alamofire.request("http://0.0.0.0:tuicool@api.tuicool.com/api/signup/register_by_email.json", method: .post, parameters: ["email":"101659109@qq.com","name":"鱼摆摆","password":"123456789"], encoding:URLEncoding.default, headers: nil).responseJSON { (response) in
    ...
    func test5()  {
        Alamofire.request("http://0.0.0.0:tuicool@api.tuicool.com/api/articles/hot.json").responseJSON { (response) in
            print(response)
            if response.result.isSuccess {
                print("请求成功2---Alamofire")
                let result = Mapper<articlesObj>().map(JSONObject: response.result.value)
                for statuses : Statuses in (result?.articles!)! {
                    print(statuses.title ?? "",statuses.id,statuses.feed_title ?? "XXX" ,statuses.abs ?? "yyy")
                }
            }else {
                print("请求失败---Alamofire")
            }
...
//            print("jsonData数组访问--\(String(describing: studentid))")
//
//            let name: AnyClass! = object_getClass(response)
//            print(name)
            //  总结 Swift: 用Alamofire做http请求，用ObjectMapper解析JSON 比较好
